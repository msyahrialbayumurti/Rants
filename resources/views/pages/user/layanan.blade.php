    ...
    <div class="container mx-auto p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white shadow-lg rounded-lg p-6 lg:col-span-2">
                ...
                <div id="calendar" class="rounded-lg border border-gray-200 shadow-sm"
                    style="min-height: 300px; max-height: 300px; overflow: hidden;"></div>
                <div id="event-details" class="p-4 bg-gray-50 mt-4 rounded-lg shadow-inner">
                    <p class="text-gray-500 italic">Klik pada jadwal untuk melihat detail.</p>
                </div>
            </div>
...
<div class="lg:col-span-2">
    ...
    <div id="produk-container" class="mt-6 p-6 bg-gray-50 rounded-lg shadow-md">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Produk yang Tersedia</h3>
        <div id="produk-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
...
<!-- Modal Pemesanan -->
<div id="modal-pesan" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Formulir Pemesanan</h3>
        <form id="form-pesan">
            <div class="mb-4">
                <label for="produk-name" class="block text-sm font-medium text-gray-700">Nama Produk</label>
                <input type="text" id="produk-name" name="produk-name" class="w-full mt-1 p-2 border rounded-lg" readonly>
            </div>
            <div class="mb-4">
                <label for="jumlah" class="block text-sm font-medium text-gray-700">Jumlah</label>
                <input type="number" id="jumlah" name="jumlah" class="w-full mt-1 p-2 border rounded-lg" min="1" required>
            </div>
            <div class="flex justify-end space-x-4">
                <button type="button" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg" id="btn-close">Batal</button>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Pesan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const produkNameInput = document.getElementById('produk-name');
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        if (data.data.length > 0) {
                            data.data.forEach(item => {
                                produkList.innerHTML += `
                                    <div class="bg-white shadow rounded p-4 relative">
                                        <img src="/storage/${item.image}" alt="${item.name}" class="w-full h-40 object-cover rounded mb-2">
                                        <h3 class="font-bold text-lg">${item.name}</h3>
                                        ${item.jumlah ? `<p>Jumlah: ${item.jumlah}</p>` : ''}
                                        ${item.warna ? `<p>Warna: ${item.warna}</p>` : ''}
                                        ${item.ukuran ? `<p>Ukuran: ${item.ukuran}</p>` : ''}
                                        <p>Harga: Rp ${item.harga}</p>
                                        <button class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg absolute bottom-4 right-4 btn-pesan" data-name="${item.name}">Pesan</button>
                                    </div>
                                `;
                            });
                            ...
                            produkList.innerHTML = '<p class="text-gray-500 italic">Tidak ada produk untuk layanan ini.</p>';
                        }
                    })
                    .catch(error => {
                        produkList.innerHTML = '<p class="text-red-500 italic">Terjadi kesalahan saat memuat produk.</p>';
                        console.error(error);
                    });
            });
        });
        ...
        btnClose.addEventListener('click', function () {
            modalPesan.classList.add('hidden');
        });
    ...
    <script>
        // Ini adalah kode PHP Blade
        var eventsFromDatabase = {!! json_encode($events) !!};
    </script>
