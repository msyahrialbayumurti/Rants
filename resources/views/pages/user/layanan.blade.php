...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Ambil data dari Laravel (variabel $events)
            var eventsFromDatabase = @json($events);
    
            // Konversi data ke format yang cocok untuk FullCalendar
            var calendarEvents = eventsFromDatabase.map(event => ({
                title: event.title,
                start: event.date,
                description: event.description,
                color: event.color || '#3b82f6' // Default warna jika tidak ada di database
            }));
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                initialView: 'dayGridMonth',
                height: 'auto', // Mengatur tinggi fleksibel
                aspectRatio: 1.5, // Rasio aspek untuk kontrol lebih
            });
            calendar.render();
        });
    ...
    <main class="bg-gray-100">
        <div class="container mx-auto p-6">
            <div class="text-center">
                <h1 class="text-3xl font-bold text-gray-700">Layanan Kami</h1>
                <p class="text-gray-500 mt-4">Penyewaan Jasa Tari, Makeup, dan Kostum</p>
            </div>
            <div id="calendar"></div>

            <div class="calendar-card p-6 w-full lg:w-1/2 bg-white rounded-lg shadow-md">
                <div id="event-details">
                    @if ($selectedDate)
                        @if ($eventOnSelectedDate)
                            <h3>Detail Acara Pada Tanggal {{ $selectedDate }}:</h3>
                            <p>{{ $eventOnSelectedDate->keterangan }}</p>
                        @else
                            <p>Tanggal ini kosong</p>
                        @endif
                    @else
                        <p>Klik pada jadwal untuk melihat detail.</p>
                    @endif
                </div>
                                ...
                                @foreach ($events as $event)
                                    {
                                        ...
                                        start: '{{ $event->tanggal }}',
                                        url: '{{ url('layanan?tanggal=' . $event->tanggal) }}'
                                    },
                                @endforeach
                            ],
                            dayClick: function(date) {
                                window.location.href = '{{ url('layanan?tanggal=') }}' + date.format();
                            }
                        });
                    });
                </script>
            ...
            <div class="lg:w-1/2">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Daftar Layanan</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="service-card bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
                        <h3 class="text-lg font-semibold text-blue-600">Jasa Tari</h3>
                        <p class="text-gray-700 mt-1">Layanan tari profesional untuk acara spesial.</p>
                    </div>
                    <div class="service-card bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                        <h3 class="text-lg font-semibold text-red-500">Makeup</h3>
                        <p class="text-gray-700 mt-1">Layanan makeup untuk pesta dan pemotretan.</p>
                    </div>
                    <div class="service-card bg-green-50 border-l-4 border-green-500 p-4 rounded-lg">
                        <h3 class="text-lg font-semibold text-green-500">Kostum</h3>
                        <p class="text-gray-700 mt-1">Sewa kostum untuk berbagai acara.</p>
                    </div>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-8">
                @foreach ($services[2]['products'] as $kostum)
                    <div class="product-card bg-white border border-gray-200 p-4 rounded-lg shadow-md">
                        <img src="{{ asset('storage/' . $kostum->image) }}" alt="{{ $kostum->nama_kostum }}"
                            class="w-full aspect-video object-cover rounded-lg mb-4">
                        <h3 class="text-lg font-semibold text-gray-800">{{ $kostum->nama_kostum }}</h3>
                        <p class="text-gray-700 mt-1">Jumlah: {{ $kostum->jumlah }}</p>
                        <p class="text-gray-700 mt-1">Warna: {{ $kostum->warna }}</p>
                        <p class="text-gray-700 mt-1">Ukuran: {{ $kostum->ukuran }}</p>
                        <p class="text-gray-700 mt-1">Harga: Rp{{ number_format($kostum->harga, 0, ',', '.') }}</p>
                        <a href="{{ url('/kostum', $kostum->id) }}"
                            class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700">Lihat
                            Detail</a>
                    </div>
                @endforeach
            </div>
    ...
    <!-- <script>
        ...
        document.querySelectorAll('.detail-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const existingDetail = parent.querySelector('.detail-view');

                if (existingDetail) {
                    existingDetail.remove();
                } else {
                    const detailElement = document.createElement('div');
                    detailElement.classList.add('detail-view');
